...
size_t halfedgeLookup(const std::vector<size_t>& compressedList, size_t target, size_t start, size_t end) {
    ...
    // ...but we don't want to degrade to O(N^2) for really high valence vertices, so fall back to a binary search
    else {
...
HalfedgeMesh::HalfedgeMesh(const PolygonSoupMesh& input, Geometry<Euclidean>*& geometry) {
    ...
    // Each vertex's neighbors are stored between vertexNeighborsStart[i] and vertexNeighborsStart[i+1]
    ...
    for(size_t iVert = 0; iVert < input.vertexCoordinates.size(); iVert++) {
        std::sort(allVertexNeighbors.begin() + vertexNeighborsStart[iVert], allVertexNeighbors.begin() + vertexNeighborsStart[iVert+1]);
    }

    
    // Count real and imaginary edges and faces and cache adjacent twin indices
    // Note: counting boundary loops is kinda difficult, so we wait to do so until the final step
    ...
    for(size_t iVert = 0; iVert < input.vertexCoordinates.size(); iVert++) {
        ...
        for(size_t jInd = jStart; jInd < jEnd; jInd++) {
            ...
            size_t searchResult = halfedgeLookup(allVertexNeighbors, iVert, vertexNeighborsStart[jVert], vertexNeighborsStart[jVert+1]);
    ...
    // Keep track of the edges we've already created since we only need one per edge
    ...
    for(auto poly : input.polygons) {
        ...
        for(size_t iPolyEdge = 0; iPolyEdge < degree; iPolyEdge++) {
            ...
            // Get a reference to the edge shared by this and its twin, creating the object if needed 
            size_t myHeInd = halfedgeLookup(allVertexNeighbors, ind2, vertexNeighborsStart[ind1], vertexNeighborsStart[ind1+1]);
    ...
    for(size_t iHe = 0; iHe < nHalfedges(); iHe++) {
        ...
        // Note: If distinct holes share a given vertex, this algorithm will see them as a single "figure 8-like"
        // hole and connect them with a single imaginary face.
        // TODO: fix this?

        // If this halfedge doesn't have a twin, it must be on a boundary (or have already been processed while walking a hole)
        if(halfedge(iHe)->twin == nullptr) {
...
HalfedgeMesh* HalfedgeMesh::copy() {
    ...
    // Copy the lazy way: Build a polygon soup mesh and construct a new halfedge mesh

    // TODO I'm not actually sure if this is a true fixed point. It certainly preserves 
