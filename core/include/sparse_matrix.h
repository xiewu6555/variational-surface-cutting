...
namespace GC
{
    ...
    class SparseMatrix
    {
        public:
            ...
            SparseMatrix<T> multiply(SparseMatrix<T>& B, bool transposeA = false, bool transposeB = false);
            // returns product of this matrix with sparse B
            ...
            void setSubMatrix(size_t r0, size_t r1, size_t c0, size_t c1, const SparseMatrix<T>& B);
    ...
    bool sameSparsity(const SparseMatrix<T>& A,
    ...
    template<typename T>
    void solveIndefinite(   SparseMatrix<T>& A,
                            DenseMatrix<T>& x,
                            DenseMatrix<T>  b);
    // solves the indefinite sparse linear system Ax = b using sparse LDL^T factorization

    template<typename T>
    void solveModifiedIndefinite(   SparseMatrix<T>& A,
                                    DenseMatrix<T>& x,
                                    DenseMatrix<T>  b);
    ...
    template<typename T>
    void solveConjugateGradient(    SparseMatrix<T>& A,
                                    SparseMatrix<T>& M,
                                    DenseMatrix<T>& x,
                                    DenseMatrix<T>& b,
    ...
    // power iterations are computed via preconditioned conjugate gradient
     ...
     template<typename T>
     void solveLeastSquares(const SparseMatrix<T>& A,
                            const DenseMatrix<T>& b,
                            DenseMatrix<T>& x);
     // solves the positive definite sparse linear system A^TAx = A^Tb using sparse Cholesky factorization
