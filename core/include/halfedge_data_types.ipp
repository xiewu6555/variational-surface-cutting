...
template<typename T>
VertexData<T>::VertexData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector, const VertexData<size_t>& indexer) 
    : VertexData(parentMesh)
{
...
template<typename T>
GC::DenseMatrix<T> VertexData<T>::toVector(const VertexData<size_t>& indexer) const {
    size_t outSize = 0;
    for(VertexPtr v : mesh->vertices()) if(indexer[v] != std::numeric_limits<size_t>::max()) outSize++;
...
template<typename T>
void VertexData<T>::fromVector(const GC::DenseMatrix<T>& vector, const VertexData<size_t>& indexer) {
    for(VertexPtr v : mesh->vertices()) {
        if(indexer[v] != std::numeric_limits<size_t>::max()) {
...
#ifndef NDEBUG
    assert(v->parentMesh == mesh && "Attempted to access data with member from wrong mesh");
#endif
...
#ifndef NDEBUG
    assert(v->parentMesh == mesh && "Attempted access data with member from wrong mesh");
#endif
    size_t i = v - mesh->vertex(0);
    return data[i];
}
...
template<typename T>
EdgeData<T>::EdgeData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector) 
    : EdgeData(parentMesh)
{
    fromVector(vector);
}
...
template<typename T>
EdgeData<T>::EdgeData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector, const EdgeData<size_t>& indexer) 
    : EdgeData(parentMesh)
{
    fromVector(vector, indexer);
}
...
template<typename T>
GC::DenseMatrix<T> EdgeData<T>::toVector(const EdgeData<size_t>& indexer) const {
    size_t outSize = 0;
    for(EdgePtr e : mesh->edges()) if(indexer[e] != std::numeric_limits<size_t>::max()) outSize++;
    GC::DenseMatrix<T> result(outSize);
    for(EdgePtr e : mesh->edges()) {
...
void EdgeData<T>::fromVector(const GC::DenseMatrix<T>& vector) {
    if(vector.nRows() != mesh->nEdges()) throw std::runtime_error("Vector size does not match mesh size.");
    EdgeData<size_t> ind = mesh->getEdgeIndices();
    for(EdgePtr e : mesh->edges()) {
        (*this)[e] = vector(ind[e]) ;
    }
...
template<typename T>
void EdgeData<T>::fromVector(const GC::DenseMatrix<T>& vector, const EdgeData<size_t>& indexer) {
    for(EdgePtr e : mesh->edges()) {
        if(indexer[e] != std::numeric_limits<size_t>::max()) {
...
#ifndef NDEBUG
    assert(e->parentMesh == mesh && "Attempted access data with member from wrong mesh");
#endif
...
template<typename T>
FaceData<T>::FaceData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector) 
    : FaceData(parentMesh)
{
    fromVector(vector);
}
...
template<typename T>
FaceData<T>::FaceData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector, const FaceData<size_t>& indexer) 
    : FaceData(parentMesh)
{
    fromVector(vector, indexer);
}
...
template<typename T>
GC::DenseMatrix<T> FaceData<T>::toVector(const FaceData<size_t>& indexer) const {
    size_t outSize = 0;
    for(FacePtr f : mesh->faces()) if(indexer[f] != std::numeric_limits<size_t>::max()) outSize++;
    GC::DenseMatrix<T> result(outSize);
    for(FacePtr f : mesh->faces()) {
...
void FaceData<T>::fromVector(const GC::DenseMatrix<T>& vector) {
    if(vector.nRows() != mesh->nFaces()) throw std::runtime_error("Vector size does not match mesh size.");
    FaceData<size_t> ind = mesh->getFaceIndices();
    for(FacePtr f : mesh->faces()) {
        (*this)[f] = vector(ind[f]) ;
    }
...
template<typename T>
void FaceData<T>::fromVector(const GC::DenseMatrix<T>& vector, const FaceData<size_t>& indexer) {
    for(FacePtr f : mesh->faces()) {
        if(indexer[f] != std::numeric_limits<size_t>::max()) {
...
#ifndef NDEBUG
    assert(f->parentMesh == mesh && "Attempted access data with member from wrong mesh");
#endif
...
template<typename T>
HalfedgeData<T>::HalfedgeData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector) 
    : HalfedgeData(parentMesh)
{
    fromVector(vector);
}
...
template<typename T>
HalfedgeData<T>::HalfedgeData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector, const HalfedgeData<size_t>& indexer) 
    : HalfedgeData(parentMesh)
{
    fromVector(vector, indexer);
}
...
template<typename T>
GC::DenseMatrix<T> HalfedgeData<T>::toVector(const HalfedgeData<size_t>& indexer) const {
    size_t outSize = 0;
    for(HalfedgePtr he : mesh->halfedges()) if(indexer[he] != std::numeric_limits<size_t>::max()) outSize++;
    for(HalfedgePtr he : mesh->imaginaryHalfedges()) if(indexer[he] != std::numeric_limits<size_t>::max()) outSize++;
    GC::DenseMatrix<T> result(outSize);
    for(HalfedgePtr he : mesh->halfedges()) {
...
void HalfedgeData<T>::fromVector(const GC::DenseMatrix<T>& vector) {
    if(vector.nRows() != (mesh->nHalfedges() + mesh->nImaginaryHalfedges())) throw std::runtime_error("Vector size does not match mesh size.");
    HalfedgeData<size_t> ind = mesh->getHalfedgeIndices();
    for(HalfedgePtr he : mesh->halfedges()) {
...
template<typename T>
void HalfedgeData<T>::fromVector(const GC::DenseMatrix<T>& vector, const HalfedgeData<size_t>& indexer) {
    for(HalfedgePtr he : mesh->halfedges()) {
        if(indexer[he] != std::numeric_limits<size_t>::max()) {
...
#ifndef NDEBUG
    assert(he->parentMesh == mesh && "Attempted access data with member from wrong mesh");
#endif
    if(he.isReal()) {
...
template<typename T>
CornerData<T>::CornerData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector) 
    : CornerData(parentMesh)
{
    fromVector(vector);
}
...
template<typename T>
CornerData<T>::CornerData(HalfedgeMesh* parentMesh, const GC::DenseMatrix<T>& vector, const CornerData<size_t>& indexer) 
    : CornerData(parentMesh)
{
    fromVector(vector, indexer);
}
...
template<typename T>
GC::DenseMatrix<T> CornerData<T>::toVector(const CornerData<size_t>& indexer) const {
    size_t outSize = 0;
    for(CornerPtr c : mesh->corners()) if(indexer[c] != std::numeric_limits<size_t>::max()) outSize++;
    GC::DenseMatrix<T> result(outSize);
    for(CornerPtr c : mesh->corners()) {
...
void CornerData<T>::fromVector(const GC::DenseMatrix<T>& vector) {
    if(vector.nRows() != mesh->nCorners()) throw std::runtime_error("Vector size does not match mesh size.");
    CornerData<size_t> ind = mesh->getCornerIndices();
    for(CornerPtr c : mesh->corners()) {
        (*this)[c] = vector(ind[c]) ;
    }
...
template<typename T>
void CornerData<T>::fromVector(const GC::DenseMatrix<T>& vector, const CornerData<size_t>& indexer) {
    for(CornerPtr c : mesh->corners()) {
        if(indexer[c] != std::numeric_limits<size_t>::max()) {
...
#ifndef NDEBUG
    assert(c->parentMesh == mesh && "Attempted access data with member from wrong mesh");
#endif
